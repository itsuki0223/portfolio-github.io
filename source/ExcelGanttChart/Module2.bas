...
Sub 作業者()
    ...
    '作業者記入
    For GYO_machine = 2 To sheet4_row
        Sheets(Sheet1).Cells(GYO_machine * 2 + 2, 2).Value = Sheets(Sheet4).Cells(GYO_machine, 2).Value
    Next GYO_machine
    ...
    For GYO = 2 To sheet2_row
        If Sheets(Sheet2).Cells(GYO, 5) = 2 Then
            If Sheets(Sheet2).Cells(GYO, 2) = "Touched" Then
                ...
                End If
                name = Sheets(Sheet2).Cells(GYO, 6).Value
                card = Sheets(Sheet2).Cells(GYO, 1).Value
                For GYO_machine = 2 To sheet4_row
                    If Sheets(Sheet4).Cells(GYO_machine, 3).Value = name Then
                        Sheets(Sheet1).Cells(GYO_machine * 2 + 3, p).Value = "0"
                        For c = GYO + 1 To sheet2_row
                            If Sheets(Sheet2).Cells(c, 1).Value = card And Sheets(Sheet2).Cells(c, 2).Value = "on" Then
                                For e = 2 To sheet3_row
                                    If Sheets(Sheet3).Cells(e, 3) = card Then
                                        machine = Sheets(Sheet3).Cells(e, 2).Value
                                    End If
                                Next e
                            ElseIf Sheets(Sheet2).Cells(c, 2).Value = "Released" And Sheets(Sheet2).Cells(c, 6).Value = name Then
                                Exit For
                            End If
                        Next c
                        For d = GYO + 1 To sheet2_row
                            If Sheets(Sheet2).Cells(d, 5).Value = 3 And Sheets(Sheet2).Cells(d, 1).Value = card And Sheets(Sheet2).Cells(d, 2).Value = "end" Then
                                pr = Sheets(Sheet2).Cells(d, 7).Value
                            ElseIf Sheets(Sheet2).Cells(d, 2).Value = "Touched" And Sheets(Sheet2).Cells(d, 1).Value = card Then
                                Exit For
                            End If
                        Next d
                        info = pr + " , " + machine
    ...
    For GYO = 7 To 20
        For RETSU = 12 To 200
            If Sheets(Sheet1).Cells(GYO, RETSU).Value = "0" Then
                If a <> "" Then
                    Sheets(Sheet1).Range(a, Cells(GYO, RETSU)).Interior.Color = RGB(0, 255, 255)
                End If
            ...
            ElseIf Sheets(Sheet1).Cells(GYO, RETSU) = "1" Then
                ...
                If a = "" Then
                    Sheets(Sheet1).Range(b).Interior.Color = RGB(0, 255, 255)
                    a = ""
                    b = ""
                Else
                    Sheets(Sheet1).Range(a, b).Interior.Color = RGB(0, 255, 255)
                    a = ""
                    b = ""
                End If
            End If
