...
class OperatorList:
	def __init__(self, Frame, sort_num, expr, id, response, person_time, raspi_sid, func, getInfo):
		...
		try:
			file = open('/home/pi/tmp_data/modeC/'+str(self.id)+'.csv', 'r')
			...
			for line in data:
				tmp_data = line
                        ...
                        self.Process = tmp_data[3]
                        self.pr_name = tmp_data[4]
			self.start_time = tmp_data[5]
			self.client = tmp_data[6]
			self.pr_time = tmp_data[7]
	...
	def buttonOperator(self, event):
		...
		operator_label.place(rely=0.24,relx=0.05, relheight=0.17, relwidth=0.45)
		...
		#作業・工程ラベル
		self.order_text = "案件: "+self.order_name
		self.order_label = Label(self.root, text=self.order_text+"　"+str(self.client), font=("",30), anchor='w')
		self.order_label.place(relx=0.05, relwidth=0.9, rely=0.43, relheight=0.17)

		self.process_text = "工程: "+self.pr_name
                self.process_label = Label(self.root, text=self.process_text, font=("",30), anchor='w')
                self.process_label.place(relx=0.05, relwidth=0.7, rely=0.62, relheight=0.17)
		self.process_time = Label(self.root, text=self.pr_time, font=("",30), anchor='e')
                self.process_time.place(relx=0.75, relwidth=0.2, rely=0.62, relheight=0.17)
		...
		button3 = Button(self.root,text='終了',font=("",25),bg='gray84',bd=5,relief=RAISED)
                button3.bind("<Button-1>",self.del_process)
	...
	def del_process(self,event):
		if self.Process != "":
			TIME = time.strftime("%Y/%m/%d %H:%M:%S")
			self.barcode_label.configure(text=self.Process)
			if config['kvs'] == "ON":
                		self.Kvs_res(self.Process,2,TIME)
			if config['csv'] == "ON":
				self.csv_write(self.Process,2,TIME)
                	self.Process = ""
                	self.pr_name = ""
                	self.process_text = "工程: "+self.pr_name
                	self.process_label.configure(text=self.process_text)
			self.pr_time = ""
			self.process_time.configure(text=self.pr_time)
			...
			self.button.configure(fg='black',bg='gray84')
	...
	def Barcode(self, event):
		...
		if expr2[0:1] == config['process']:
			expr2 = expr2[1:]
			if expr2 == self.Process:
				self.barcode_label.configure(text=self.Process)
				if config['kvs'] == "ON":
					self.Kvs_res(self.Process,2,TIME)
				if config['csv'] == "ON":
				...
				try:
					...
					response = self.getInfo(pr_path, expr2)
					self.pr_name = response[u'工程機械名称']
					self.Process = expr2
					print(self.pr_name)
					self.start_time = TIME
					if config['kvs'] == "ON":
                                        	self.Kvs_res(self.Process,1,TIME)
                                        if config['csv'] == "ON":
                                        	self.csv_write(self.Process,1,TIME)
					print(self.Process)
					self.barcode_label.configure(text=self.Process)
					self.process_text = "工程: "+self.pr_name
					self.process_label.configure(text=self.process_text)
	...
	def tmp_csv(self):
		f = open('/home/pi/tmp_data/modeC/'+str(self.id)+'.csv','w')
		data = [self.order, self.Order, self.order_name, self.Process, self.pr_name, self.start_time, self.client, self.pr_time]
		print(data)
		writer = csv.writer(f)
		writer.writerow(data)
		f.flush()
	...
	def delete_ins(self):
		...
		subprocess.call(['sudo','rm','/home/pi/tmp_data/modeC/'+str(self.id)+'.csv'])	
		try:
			self.button.destroy()
		except:
			pass
